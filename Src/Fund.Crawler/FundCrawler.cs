...
namespace Fund.Crawler
{
    ...
    public class FundCrawler
    {
        ...
        /// <returns></returns>
        public async Task<List<FundInfo>> StartFund(CancellationToken token, params string[] fundIds)
        {
        ...
        /// <returns></returns>
        public async Task<List<IndexInfo>> StartIndex(CancellationToken token, params string[] indexCodes)
        {
            return await Task.Run(() =>
            {
                ...
                if (indexCodes != null)
                {
                    ...
                    Parallel.ForEach(indexCodes, (indexCode, pls, index) =>
                    {
                        if (token.IsCancellationRequested)
                        {
                            WebCrawler.WriteLog($"爬取指数相关基金信息{indexCode}任务已取消");
                            return;
                        }
                        result.Add(WebCrawler.Start(new IndexKey(index, indexCode), token).Result);
                    });

                    if (token.IsCancellationRequested)
                    {
                        WebCrawler.Cancel("爬取指数相关基金信息任务已取消");
                    }
                    else
                    {
